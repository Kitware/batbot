...
def test_small():
    weights = np.array(
        [[1, 3, 3, 3, 3], [2, 1, 3, 3, 3], [2, 2, 1, 3, 3], [2, 2, 2, 1, 3], [2, 2, 2, 2, 1]],
        dtype=np.float32,
    )
...
def test_orthogonal_x():
    ...
    path = pyastar2d.astar_path(
        weights, (0, 0), (4, 4), allow_diagonal=False, heuristic_override=Heuristic.ORTHOGONAL_X
    )
...
def test_orthogonal_y():
    weights = np.ones((5, 5), dtype=np.float32)
    path = pyastar2d.astar_path(
        weights, (0, 0), (4, 4), allow_diagonal=False, heuristic_override=Heuristic.ORTHOGONAL_Y
    )
