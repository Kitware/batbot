...
def test_preprocess():
    ...
    for ii in range(num_examples):
        ...
        assert any([expected_file in x for x in output_str]), 'Did not find file listed among outputs: {}'.format(expected_file)
        assert os.path.exists(expected_file), 'Did not find file on filesystem: {}'.format(expected_file)
    ...
    for ii in range(num_examples):
        ...
        err_str = 'Expected at least {} bat call segments in file {}, found only {} segments'.format(
            num_min_call_segments[ii],
            expected_file,
            n_segments
            )
